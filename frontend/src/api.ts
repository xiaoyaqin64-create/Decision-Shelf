...
export const api = {
  ...
  updateCompletion: (id: string, patch: { completed_at: string; rating: number | null; review: string | null }) => request<Card>(`/api/cards/${id}/completion`, { method: 'PATCH', body: JSON.stringify(patch) }),
  enrichCard: (id: string) => request<{ draft: CardDraft; source: string; warning: string | null; retried: boolean; description_source: string | null }>(`/api/cards/${id}/enrich`, { method: 'POST' }),
  ...
  enrich: (draft: CardDraft) => request<{ draft: CardDraft; source: string; warning: string | null; retried: boolean; description_source: string | null }>('/api/metadata/enrich', { method: 'POST', body: JSON.stringify({ draft }) }),
